...
AquaCode API is designed to be the engine that connects all of AquaCode’s services.
...
### Auto-generated API (Schemas → Endpoints)
- Endpoints dynamically generated based on the schemas defined in each project’s model.
...
Click **Try it out → Execute** to create the initial user.
...
- The API exposes several internal routes used by AquaCode Studio — not all are intended for external use.
