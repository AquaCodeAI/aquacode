...
export class FileDto {
	/**
	 * Unique source code file identifier
	 * @example 'scf_01JV5V3CVXQNYFM2NHCRBM7KQH'
	 */
	@ApiProperty({
		type: String,
		description: 'Unique source code file identifier in ULID format.',
		required: true,
		example: 'scf_01JV5V3CVXQNYFM2NHCRBM7KQH',
		pattern: '^scf_[0-9A-Za-z]{26}$',
		minLength: 30,
		maxLength: 30,
	})
	@Expose()
	@IsString({ message: 'Source code file ID must be a string' })
	@IsNotEmpty({ message: 'Source code file ID is required' })
	@MinLength(30, {
		message: 'Source code file ID must be exactly 30 characters long',
	})
	@MaxLength(30, {
		message: 'Source code file ID must be exactly 30 characters long',
	})
	@Matches(/^scf_[0-9A-Za-z]{26}$/, {
		message:
			'Source code file ID must be in ULID format: "scf_" followed by 26 alphanumeric characters',
	})
	_id: string;
